...
samples <- apply(truemeans, c(1,2), function(x) rgamma(1, x, 1))
write.csv(samples, file = "gamma_samples.csv", row.names = FALSE)

# 2) Now fit using SSQ and WLS
setwd("../")
source("general.R")
tmp_ssq <- run_model("simulated_data/gamma_samples.csv", model = "full", skipEM = FALSE)
tmp_wls <- run_model("simulated_data/gamma_samples.csv", model = "full", goalf = "WLS", skipEM = FALSE)
...
getlik <- function(pars, n, nrep){
  ...
  ll <- dgamma(samples, newmeans, 1, log = TRUE)
  ll[is.infinite(ll)] <- 0
...
Bgamma <- matrix(tmp$par, n, n)
pred_gamma <- getpred(Bgamma, n, nrep)
plot(as.vector(B), as.vector(Bgamma)); abline(c(0,1))
...
# FIGURE OBS ABUNDANCE VS PREDICTED
...
ggsave(pl, filename = "Gamma_predabundance_v_observed.pdf", width = 7, height = 3)
...
toplot2 <- tibble(`true parameters` = as.vector(B),
                  `inferred parameters` = as.vector(Bgamma), 
                  `goalf` = "Gamma")
...
ggsave(pl2, filename = "Gamma_pars.pdf", width = 7, height = 3)
