...
# \tilde{x}_i^(k, r) ~ Gamma(x_i^(k), 1)

# n number of species in the pool
# rep number of replicates per community
generate_gamma_simulated <- function(seed, n, rep){
...
# Now sample from Gamma distributions with shape x^(k)_i and rate 1
Xs <- matrix(0, 0, n)
Xg <- matrix(0, 0, n)
for (i in 1:nrow(X)){
  x <- X[i, ]
  xs <- x[x > 0]
  for (j in 1:rep){
    xsr <- rgamma(n = length(xs), shape = xs, rate = 1)
    tmp <- x
    tmp[x > 0] <- xsr
    Xs <- rbind(Xs, tmp)
    Xg <- rbind(Xg, x)
...
write.csv(Xs, file = paste0("../data/simulated_data/gamma_n_", n, "_rep_", rep, ".csv"), row.names = FALSE)
